...
; 2.2.3.2.2 Que guarde correctamente con caracteres especiales
...
Local $violacion = "Violaciones al derecho de los pueblos indÌgenas a la tierra y territorio"
Local $notas = "notas de una legislacion nacional de un acto de un caso de pruebas  ·ÈÌÛ˙¡…Õ”⁄Ò~`!@#$%^&*()_-+={}[]|\:;'<>,.?/ø°"
...
Local $items = SMDH_ManejoDeCasos_Actos_Get_LegislacionesNacionales($victima, $violacion)
For $i = 0 To 3 ;UBound($items) - 1
	SMDH_ManejoDeCasos_Actos_Set_LegislacionNacional($victima, $violacion, $items[$i])
	SMDH_ManejoDeCasos_Actos_Set_LegislacionNacional_Notas($victima, $violacion, $items[$i], $notas & "[" & String($i) & "]")
	SMDH_ManejoDeCasos_Actos_Remove_LegislacionNacional($victima, $violacion, $items[$i])
Next
