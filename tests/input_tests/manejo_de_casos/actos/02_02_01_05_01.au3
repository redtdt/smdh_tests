...
; 2.2.1.5.1 Que guarde correctamente la selección (probar todas las opciones)
...
Local $items = SMDH_ManejoDeCasos_Actos_Get_CaracteristicasRelevantes($victima, $violacion)
;For $i = 0 To UBound($items) - 1
;	SMDH_ManejoDeCasos_Actos_Add_CaracteristicaRelevante($victima, $violacion, $items[$i])
;	SMDH_ManejoDeCasos_Actos_Remove_CaracteristicaRelevante($victima, $violacion, $items[$i])
;Next
For $i = 0 To UBound($items) - 1
	SMDH_ManejoDeCasos_Actos_Add_CaracteristicaRelevante($victima, $violacion, $items[$i])
Next
