...
; 2.5.2.7.1 Que guarde correctamente la selección (probar todas las opciones)
...
Local $EstadosCiviles = SMDH_Personas_Individual_Get_EstadosCiviles($nombre, $apellido)
For $EstadoCivil = 0 To UBound($EstadosCiviles) - 1
	SMDH_Personas_Individual_Set_EstadoCivil_Idx($nombre, $apellido, $EstadoCivil)
Next
SMDH_ManejoDeCasos_Personas_DatosGenerales_Open()
...
SMDH_ManejoDeCasos_Personas_Detalles_Open()
Local $EstadosCiviles = SMDH_Personas_Colectiva_Get_EstadosCiviles($colectiva, $sigla)
For $EstadoCivil = 0 To UBound($EstadosCiviles) - 1
	SMDH_Personas_Colectiva_Set_EstadoCivil_Idx($colectiva, $sigla, $EstadoCivil)
Next
SMDH_ManejoDeCasos_Personas_DatosGenerales_Open()
